...
支持 **CLI** 和 **MCP 服务**两种使用方式，共 23 个 MCP 工具。
...
| `generate_ore_texture` | 简化矿石生成：自动使用 ore_background/overlay，仅需颜色 | name, color, deepslate(bool) |
...
基于灰度模板图着色，生成 Minecraft 木质方块纹理。模板为 16×16 灰度图，通过 `colorize_grayscale` 按亮度乘法着色。

| 工具 | 说明 | 关键参数 |
|------|------|----------|
| `generate_sapling` | 树苗（body + leaves 独立着色后合成）| name, body_color, leaves_color |
| `generate_planks` | 木板 | name, color |
| `generate_log` | 原木 + 原木顶（返回两个路径）| name, color |
| `generate_stripped_log` | 去皮原木 + 去皮原木顶 | name, color |
| `generate_leaves` | 树叶 | name, color |
| `generate_door` | 门上半 + 门下半 | name, color |
| `generate_trapdoor` | 活板门 | name, color |

#### 分析类

| 工具 | 说明 | 关键参数 |
|------|------|----------|
| `analyze_image_colors` | 提取图片主要颜色（RGB 分桶量化）| input_path, max_colors(1-12) |
...
```
AI 生成：API 生成原始图 → 去背景 → NEAREST 缩放到目标尺寸 → 保存 PNG
图像处理：加载图片 → 去背景 → NEAREST 缩放 → 保存 PNG（不调 AI）
```

- **去背景**：三级管线 —
  1. 四边采样检测背景色 + 动态容差（p90 色差）。适合背景均匀、主体不触边的图。
  2. 直方图后备：边缘采样失败时（p90 > 80），用全图颜色直方图找主色调作为背景色。
  3. 连通分量去噪：BFS 找不透明连通区，清理孤立噪点碎片（< 0.2% 画布面积）。
...
├── mcp_server.py             # MCP 服务（23 个工具）
├── api_token.txt             # API 配置（不入 git）
├── api_token.example.txt     # 配置模板
├── requirements.txt          # Python 依赖
├── mcp_config.json           # MCP 配置参考
├── ore_background.png         # 普通石头底图（16x16）
├── deepslate_ore_background.png # 深板岩底图（16x16）
├── ore_overlay.png            # 矿石纹路叠加层（16x16）
├── sapling_body.png           # 树苗枝干模板（16x16 灰度）
├── sapling_leaves.png         # 树苗树叶模板（16x16 灰度）
├── plansk.png                 # 木板模板（16x16 灰度）
├── log.png / log_top.png      # 原木模板（16x16 灰度）
├── stripped_log.png / stripped_log_top.png  # 去皮原木模板
├── leaves.png                 # 树叶模板（16x16 灰度）
├── door_bottom.png / door_top.png # 门模板（16x16 灰度）
├── trapdoor.png               # 活板门模板（16x16 灰度）
├── README.md
├── test/                      # 测试输出（不入 git）
└── tool/                      # API 格式与提示词参考
```
